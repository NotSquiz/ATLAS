...
## D100: BB Primary Interface — MCP Tools + CLI
**Date:** January 30, 2026
**Context:** How should users interact with BB automation?

**Decision:** Claude Code MCP tools (primary) + CLI (secondary) + Telegram (future) + voice intent (future)

**Rationale:**
- User already lives in Claude Code — adding tools to existing MCP server requires zero new tooling
- CLI provides fallback for scripts and cron jobs
- 5 MCP tools implemented: bb_status, bb_find_doc, bb_warming_daily, bb_warming_done, bb_warming_status

---

## D101: Dual-Mode Claude — Max for Interactive, API for Automation
**Date:** January 30, 2026
**Context:** Cost optimization for BB operations

**Decision:** Claude Code CLI via Max ($0) for interactive work; Anthropic API (Sonnet) for automated cron/daemon tasks

**Rationale:**
- Interactive work (strategy, review): Claude Code CLI via Max subscription = $0
- Automated tasks (comment generation, transcript analysis): Sonnet API, token-optimized = ~$1-3/mo
- Total monthly API cost estimate: ~$8-15

---

## D102: Warming Pipeline — Auto-Watch + Human-Post Comments
**Date:** January 30, 2026
**Context:** Two audits confirmed automated comment posting = permanent account ban on YouTube/Instagram

**Decision:** Auto-watch/like/subscribe via Playwright; comments generated by Sonnet but HUMAN-POSTED (copy-paste)

**Rationale:**
- YouTube AI moderation detects browser automation at infrastructure level (Dec 2025 strengthening)
- Accounts terminated without human review — existential risk for pre-launch brand
- ~15 min/day human effort for comment posting
- Playwright for watch/like/subscribe carries very low risk (passive engagement)

---

## D103: BB Comment System — Three-Layer Knowledge-Backed Generation
**Date:** January 30, 2026
**Context:** Comments must sound human, be transcript-aware, and carry BB brand voice

**Decision:** Three-layer system: (1) Knowledge Base (transcript + cross-repo), (2) Voice Spec (BabyBrains-Writer.md), (3) Human Story (~35% personal angle)

**Rationale:**
- Layer 1 ensures comments reference actual video content (not generic)
- Layer 2 enforces AU English, anti-superlative, em-dash prohibition, Grade 8 reading level
- Layer 3 adds authenticity when human story profile is completed (PLACEHOLDER until user fills in)
- Quality gate: em-dash check, AU vocabulary, length, emoji count

**Files:**
- `atlas/babybrains/warming/comments.py` — CommentGenerator + quality gate
- `atlas/babybrains/voice_spec.py` — Section-level extraction from 1712-line spec
- `config/babybrains/human_story.json` — Human story profile (INCOMPLETE)

---

## D104: Cross-Repo Knowledge Map — Static Path Index
**Date:** January 30, 2026
**Context:** BB content spans 5 repos; orchestrator needs to know where everything is

**Decision:** Static JSON path map (`config/babybrains/cross_repo_paths.json`) with keyword search

**Rationale:**
- 16 topic mappings across 5 repos (ATLAS, babybrains-os, knowledge, web, app)
- Keyword-scored search with deduplication and file existence verification
- Static map is sufficient for current scale; can add FTS or embeddings later if needed
- No runtime dependency on having all repos cloned (graceful degradation)

---

## D105: Platform Algorithm Configs — Structured from Dec 2025 Research
**Date:** January 30, 2026
**Context:** YouTube/Instagram/TikTok/Facebook each have different algo requirements
...
- All configs in `config/babybrains/platforms.json`

---

## D106: BB Database — 10 Tables Extending ATLAS Shared DB
**Date:** January 30, 2026
**Context:** BB needs persistent storage for warming, trends, and content pipeline

**Decision:** 10 new tables (`bb_*` prefix) in the existing ATLAS SQLite database

**Rationale:**
- Reuses existing MemoryStore connection and infrastructure
- `bb_` prefix avoids namespace collisions with existing health/memory tables
